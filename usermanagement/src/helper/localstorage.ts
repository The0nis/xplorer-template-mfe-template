...
export const addRoleToLocalStorage = (roleId: string): void => {
  const encryptedUser = encrypter(roleId);
  localStorage.setItem("newRoleId", encryptedUser);
};

export const getRoleFromLocalStorage = (): string | null => {
  const result = localStorage.getItem("newRoleId");
  const decryptedUser = result ? deCrypter(result) : null;
  return decryptedUser;
};
