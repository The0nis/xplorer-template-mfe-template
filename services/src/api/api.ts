...
const requestInterceptor = (
  ...
  const getToken = localStorage.getItem('accessToken') || getUserFromLocalStorage()?.AccessToken;
...
apiWithoutToken.interceptors.request.use(
  (config: CustomRequestConfig) => requestInterceptor(config, true, false, true), // Encryption enabled, Authorization skipped
  (error) => Promise.reject(error)
);
