...
const setupInterceptors = () => {
  ...
  const requestInterceptor = (
    ...
    if (needEncryption) {
      ...
      config.data =
        typeof config.data == "string" ? config.data : encrypter(config.data);
    } else {
  ...
  const apiWithEncryptionNoDecryption = axios.create({
    baseURL: import.meta.env.VITE_API_URL,
  ...
  apiWithEncryptionNoDecryption.interceptors.response.use(
    (response: any) => response, // simply return the response without decryption
    (error) => Promise.reject(error)
  );


  const responseInterceptor = (response: any) => {
    // console.log("response-before-decryption", response);
    // response.data = deCrypter(response.data);
    // console.log("response.data-after-decryption", response);
    if (response.config.responseType === "blob") {
      return response;
    }
    response.data = deCrypter(
